...
for CODEC in g723 g729 ; do
	cp -vf "codec_${CODEC}-ast100-gcc4-glibc-${CPU_SUFFIX}.so" "${HOME}/lib/asterisk/modules/"
	ln -svnf "codec_${CODEC}-ast100-gcc4-glibc-${CPU_SUFFIX}.so" "${HOME}/lib/asterisk/modules/codec_${CODEC}.so"
done
